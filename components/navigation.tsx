    ...
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl bg-white/70 backdrop-blur-md border border-gray-200/50 z-50 shadow-xl rounded-2xl">
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 md:h-20">
          ...
          >
            <span className="text-2xl font-bold text-[#0d5f3f]">AR Energy</span>
          </Link>
            ...
            {Object.entries(navItems).map(([title, items]) => (
              <div key={title} className="relative">
                <button
                  ...
                  className={`flex items-center gap-1 font-medium transition-all duration-300 ease-out px-3 py-2 rounded-lg ${isActive(items[0].href)
                  ...
                  <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    {items.map((item) => (
                      <Link
                        key={item.href}
                        href={item.href}
                        className={`block px-4 py-2 font-normal transition-all duration-200 ${isActive(item.href)
                  ...
                  <div className="pl-2 mt-1 space-y-1 animate-in slide-in-from-top-2 duration-200">
                    {items.map((item) => (
                      <Link
                        key={item.href}
                        href={item.href}
                        className={`block px-4 py-2.5 rounded-lg transition-all duration-200 ${isActive(item.href)
